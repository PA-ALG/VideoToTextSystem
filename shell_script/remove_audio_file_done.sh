...
# 指定含有子文件夹的文件夹路径
DEST_DIR="$PARENT_DIR/files/output_audio"
# 指定含有子文件夹的文件夹路径
DEST_DIR="$PARENT_DIR/files/split_audio"
# 指定含有子文件夹的文件夹路径
DEST_DIR="$PARENT_DIR/files/results"
...
for dir in "$DEST_DIR"/*; do
  if [ -d "$dir" ]; then
    ...
    # 检查子文件夹名称是否在文件名列表中
    if [[ -n ${file_names["$dir_name"]} ]]; then
      # 如果在列表中找到了匹配的子文件夹名称，则删除该子文件夹
      rm -rf "$dir"
      echo "Deleted matching folder: $dir"
    fi
  fi
done
